...
通过pypy虽然能够加快python代码的执行，但是由于第三方库regex不支持，我们只能放弃。而通过cython，我们没有怎么修改python代码(不过为了模块调用进行了一定代码的重构)的情况下达到了最快的速度。当然我们还可以进一步优化cython代码，比如使用c++的unordered_map替代python的dict。不过更好的方法是直接在c++语言里完成这些优化，然后把它封装成动态库供cython调用。这就是下一篇文章我们要研究的内容。
