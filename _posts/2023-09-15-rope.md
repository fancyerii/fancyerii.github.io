...
上式推导的重点是：两个向量(复数表示)的内积等于第一个复数的共轭乘以第二个复数(或者第二个复数乘以第一个复数的共轭)的**实部**。这里需要注意：我们之前要找的g是两个向量的内积，因此它是一个数，但是这里怎么又变成一个向量(复数)了呢？因为我们需要找一个g是$Re[f_qf_k*]$，那么我们可以假设存在一个复数函数g，这个函数的输入是$x_q,x_k,(m-n)$，输出是一个复数。如果这样的函数g存在，那么我们的目的就达到了！！！回顾一下，如果g等于$[f_qf_k*]$并且只与m-n有关，而与m和n的绝对值无关，那么我们取g的实部，显然它也是只与m-n有关，因此$f_q$和$f_k$的内积就是我们想要的定义。
