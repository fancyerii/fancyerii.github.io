...
根据[维基](https://en.wikipedia.org/wiki/JGraph)，JGraph是2002年发布了最早的版本，是Java实现的。到了JGraph 5.x后使用JavaScript实现，更名为mxGraph，而原来的Java版本更名为JGraphX并且版本与mxGraph保持一致。Java和JavaScript版本的API是类似的，但是在今天B/S架构为主，因此更多的人是使用mxGraph。目前mxGraph(包括JGraphX)的最新版本是4.0.0。
...
可以对图做一些聚类分析，也可以计算节点的最短路径等。比如下图是最短路径。
...
我们需要在某个Element里显示mxGraph，通常当然是放到一个DIV里。我们在HTML的BODY里定义一个DIV，然后在构造mxGraph的时候通过getElementById找到它并传入，后面会看到。
